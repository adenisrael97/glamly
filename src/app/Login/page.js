...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(false);
    router.push(role === "stylist" ? "/stylist" : "/");
  };
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 sm:px-10 py-14 bg-white">
        <div className="w-full max-w-md">
          ...
          <p className="text-gray-500 text-sm mb-7">
            Sign in to your account — choose your role below.
          </p>
          ...
          <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full mt-1 py-3 bg-purple-600 hover:bg-purple-700 active:bg-purple-800 disabled:opacity-60 disabled:cursor-not-allowed text-white font-semibold rounded-lg transition-all duration-200 flex items-center justify-center gap-2 text-sm shadow-md hover:shadow-lg"
            >
              {loading && <SpinnerIcon />}
              {loading
                ? "Signing in…"
                : `Sign in as ${role === "stylist" ? "Stylist" : "Customer"}`}
            </button>
          </form>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            {["Google", "Facebook"].map((provider) => (
              <button
                key={provider}
                type="button"
                className="flex items-center justify-center gap-2 py-2.5 rounded-lg border border-gray-300 text-sm font-medium text-gray-700 hover:bg-gray-50 hover:border-gray-400 transition-all duration-200"
              >
                {provider === "Google" ? (
