...
- ✅ **D2 — Customer profile management — FIXED this pass.** Added `PATCH /auth/me` (shared schema → repo →
  service → controller), extended `AuthUser` with phone/address, and a Profile section on `/dashboard` (display +
  edit). Verified live (UI edit → 200 → DB; empty patch → 400). *Remaining polish: also collect phone/address at
  registration so the very first booking has them.*
- ✅ **D5 — Stylist-status gating — FIXED this pass.** `toAuthUser` embeds `stylistStatus`; new `/studio/pending`
  page; the stylist layout confines PENDING/SUSPENDED/REJECTED stylists there. Verified live (pending → redirect;
  approved → studio). *Remaining polish: hide studio nav links on the pending page (currently they bounce back).*
...
3. **Customers can't be reached for home-service appointments (D2).** New customers had no way to set a phone or
   address, yet stylists rely on exactly that to deliver. *(Addressed in this pass — dashboard profile + `PATCH /auth/me`;
   residual: also capture them at registration.)*
4. **Suspended/unapproved stylists keep their studio (D5).** A stylist suspended for cause (fraud, complaints) still
   saw the full studio with no indication; trust/moderation gap. *(Addressed in this pass — `/studio/pending` gate; was
   already mitigated server-side since they can't be booked/listed.)*
